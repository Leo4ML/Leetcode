...
移动顺序由字符串表示。字符 move[i] 表示其第 i 次移动。机器人的有
效动作有 R（右），L（左），U（上）和 D（下）。如果机器人在完成所有动作后返回原点，则返回 true。否则，返回 false。
...
class Solution:
    # 最初的思路，遍历一遍，计算x方向y方向是否最后都归0，时间复杂度O(n)
    # 题解也基本是O(n), 有个利用python的函数counters
